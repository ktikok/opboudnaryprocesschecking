...
G4VPhysicalVolume* DRsimDetectorConstruction::Construct() {
  ...
  fiberA = new G4Tubs("fiberA",0,clad_A_rMax,towerH/2.,0*deg,360.*deg);
  ...
  dimB->Rbool(1);
  fulltheta = 0.;
  Barrel(towerLogicalBR,PMTGLogicalBR,PMTfilterLogicalBR,PMTcellLogicalBR,PMTcathLogicalBR,fiberLogica_BR,fiberLogical_BR,fiberLogical_BR_,fTowerBR);

  dimB->Rbool(0);
  fulltheta = 0.;
  Barrel(towerLogicalBL,PMTGLogicalBL,PMTfilterLogicalBL,PMTcellLogicalBL,PMTcathLogicalBL,fiberLogica_BL,fiberLogical_BL,fiberLogical_BL_,fTowerBL);
...
void DRsimDetectorConstruction::Barrel(G4LogicalVolume* towerLogical[], G4LogicalVolume* PMTGLogical[], G4LogicalVolume* PMTfilterLogical[], G4LogicalVolume* PMTcellLogical[], G4LogicalVolume* PMTcathLogical[], 
                                       std::vector<G4LogicalVolume*> fiberLogica[], std::vector<G4LogicalVolume*> fiberLogical[], std::vector<G4LogicalVolume*> fiberLogical_[], std::vector<DRsimInterface::DRsimTowerProperty>& towerProps) {
  ...
  for (int i = 0; i < sNumBarrel; i++) {
    ...
    dimB->Getpt(pt);
    fiberBarrel(i,fDThetaBarrel[i],towerLogical,fiberLogica,fiberLogical,fiberLogical_);
...
void DRsimDetectorConstruction::fiberBarrel(G4int i, G4double deltatheta_,G4LogicalVolume* towerLogical[], 
                                            std::vector<G4LogicalVolume*> fiberLogica[], std::vector<G4LogicalVolume*> fiberLogical[], std::vector<G4LogicalVolume*> fiberLogical_[]) {
  ...
  for (unsigned int j = 0; j<fFiberX.size();j++) {
    ...
    if ( !fFiberWhich.at(j) ) { //c fibre
      ...
      // air
      // intersec = new G4IntersectionSolid("fiber_",fiberA,tower,0,G4ThreeVector(-fFiberX.at(j),-fFiberY.at(j),0.));
      // fiberLogica[i].push_back(new G4LogicalVolume(intersec,FindMaterial("G4_AIR"),name));
      // new G4PVPlacement(0,G4ThreeVector(fFiberX.at(j),fFiberY.at(j),0), fiberLogica[i].at(j),name,towerLogical[i],false,j,checkOverlaps);
      ...
      // new G4PVPlacement(0,G4ThreeVector(0.,0.,0.), fiberLogical[i].at(j),name,fiberLogica[i].at(j),false,j,checkOverlaps);
      intersect = new G4IntersectionSolid("fiber_",fiber,tower,0,G4ThreeVector(-fFiberX.at(j),-fFiberY.at(j),0.));
      fiberLogical[i].push_back(new G4LogicalVolume(intersect,FindMaterial("FluorinatedPolymer"),name));
